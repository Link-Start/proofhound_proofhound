...
| 鉴权   | HTTP 入口双通道（API `Authorization: Bearer ph_*` user token / UI 部署层 trusted header 或 LOCAL_ACTOR 兜底）；MCP 入口 user token；Webhook 入口 per-connector webhook token；OSS 不内置登录系统，部署形态 A/B/C 详见 [08](docs/specs/08-saas-adapter-boundary.md) |
...
常用命令（pnpm@10 + turbo 编排）：

| 用途                                    | 命令                                                          |
| --------------------------------------- | ------------------------------------------------------------- |
| 起全栈本地开发（docker + migrate + 5 服务） | `pnpm dev`                                                     |
| 单服务                                  | `pnpm dev:server` / `dev:web` / `dev:worker` / `dev:webhook`  |
| 类型检查 / Lint                         | `pnpm typecheck` / `pnpm lint`（`lint:fix` 自动修）           |
| 测试                                    | `pnpm test`（= test:unit） / `pnpm test:e2e`                  |
| Migration                               | `pnpm db:generate`（生成） / `pnpm db:migrate`（应用）        |
| 重置 / 种子库                           | `pnpm db:reset` / `pnpm db:seed`                              |
| 全量门禁                                | `pnpm ci` = `typecheck + lint + test + deps:check + spec:terms` |
| 循环依赖 / 术语检查                     | `pnpm deps:check`（madge） / `pnpm spec:terms`                |

> 首次运行：`cp .env.example .env` 后填 `DATABASE_URL` / `REDIS_URL`；`MODEL_API_KEY_ENCRYPTION_KEY`（@proofhound/crypto 加解密 API Key）、`MCP_TOKEN_SIGNING_SECRET`（MCP token 签名）为应用自管密钥，缺失会导致启动 / 调用失败。
...
| 改快速开始 / 设置页                     | [33](docs/specs/33-quick-start.md) + [34](docs/specs/34-settings.md)                                                                                                          |
| 改入口鉴权 / token 体系 / SaaS adapter 接口 | [08](docs/specs/08-saas-adapter-boundary.md)                                                                                                                                  |

## 4. OSS / SaaS 边界

- 当前仓库优先服务开源版 self-hosted 的完整可用闭环；新增架构必须能解释它如何改善当前 OSS 功能、可维护性或本地数据边界。
- 可以保留 `project_id`、`ProjectContext`、`ActorContext`、`accessControl`、Provider 接口、API / MCP 边界等薄接口，作为未来外部 SaaS 控制面接入点；这些接口必须有开源版默认实现，并被当前代码路径真实使用。adapter 扩展点的完整契约（`ProjectContextResolver` / `ActorContextResolver` / `McpAuthResolver` / `ConnectorContextResolver` / `TokenService` / `AccessControlService` / `LimiterKeyStrategy` / `WorkflowAuthorizationHook`）见 [08 控制面 Adapter 边界](docs/specs/08-saas-adapter-boundary.md)。
...
- 不要把任何入口 token（user token / webhook token）复用外部 JWT；Token 由应用层自管。
- 不要让 user token 与 webhook token 复用同一 scope / 同一 resolver；user token (`scope='user'`) 共用 HTTP API + MCP 入口，webhook token (`scope='webhook'`) 仅用于对应 connector 入站，两者凭证体系互相独立。
