...
describe('LocalMcpAuthResolver', () => {
  ...
  beforeEach(() => {
    verifier = { verify: vi.fn().mockResolvedValue({ actorId: 'tok-m', actorKind: 'user_token' }) };
  ...
  it('从 metadata.authInfo.token 提取并校验', async () => {
    ...
    expect(actor).toEqual({ actorId: 'tok-m', actorKind: 'user_token' });
    expect(verifier.verify).toHaveBeenCalledWith('ph_tok_a');
  });

  it('从 metadata.headers Authorization Bearer 提取', async () => {
    await resolver.resolveFromMcp({ headers: { authorization: 'Bearer ph_tok_b' } });
    expect(verifier.verify).toHaveBeenCalledWith('ph_tok_b');
  });

  it('从 metadata.meta.token 提取', async () => {
    await resolver.resolveFromMcp({ meta: { token: 'ph_tok_c' } });
    expect(verifier.verify).toHaveBeenCalledWith('ph_tok_c');
  });
  ...
  it('resolveFromUserToken 直接走 verifier 不传 IP', async () => {
    await resolver.resolveFromUserToken('ph_tok_d');
    expect(verifier.verify).toHaveBeenCalledWith('ph_tok_d');
  });
});
