...
> A large-file dataset that is being imported is **not written to the `datasets` table** (see [§3.1](#31-uploading-data), large-file streaming batched import). In-progress import sessions **do not appear on the dataset list page**: imports are driven by the upload page's frontend loop, and the user must stay on that page until completion (see [§3.1.2](#312-large-file-streaming-batched-import)), so import progress and visibility are handled by the upload page itself, and the list level does not fold in "in-progress import sessions". Only after `complete` succeeds does a dataset appear in the list as a formal dataset. Failed / interrupted sessions are deleted outright, leaving nothing behind.
...
**There is no hard upper limit on a dataset's total sample count**; large files are fully ingested via the streaming batched path below. Only the synchronous path keeps a protective threshold on the "number of samples per request", and exceeding it guides the user to the large-file import—this is request-body protection, not a dataset-size limit. The original uploaded file is not retained as a source of business truth; the ingested `dataset_samples.data` is the only data source for frontend display and experiment runs.
...
3. The frontend parses the file line by line in a streaming fashion, and each time it accumulates a batch it submits it to `POST /dataset-imports/:id/batch`; the server writes this batch of samples into the staging table `dataset_import_samples` (deduplicated by `(import_id, row_index)`, with idempotent resubmission of a single batch) and advances the session's collected-row count and heartbeat time. Batches must be submitted serially to guarantee that what has been ingested is a continuous prefix.
