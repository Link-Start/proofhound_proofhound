...
// Output format section builder — auto-generates the localized "Output Format" instruction from prompt_version.outputSchema.
//
// Design intent (see docs/specs/23-prompts.md self-describing principle):
// prompt body / variables / output_schema / judgment_rules / prompt_language are the version's execution contract;
// the body must not duplicate
// the semantics of output_schema. The newPromptBody generated by optimization only carries "task / role / guidelines / examples";
// at runtime this module auto-assembles the output-format section from outputSchema, ensuring the output contract stays stable.
//
// Only recognizes the most common JSON Schema (draft-7 compatible) shape: type=object + properties;
// for schemas that cannot be recognized, fall back to wrapping the raw JSON.stringify in a ```json code block.
